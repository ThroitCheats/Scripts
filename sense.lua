...
function EspObject:Construct()
	...
	self.drawings = {
		...
		visible = {
			...
			name = self:_create("Text", { Text = self.player.Name or "Player", Center = true, Visible = false }),
...
function EspObject:Render()
	...
	local interface = self.interface;
	local oldOptions = self.options;
	local options = {} 
	for i,v in pairs(oldOptions) do options[i] = v end
	local corners = self.corners;
	local DESTROY = false
	if (typeof(self.player) ~= "table") or not (self.player and self.player.model and self.player.model:IsDescendantOf(game)) then 
		enabled = false
		DESTROY = true
	end
	if enabled and self.player and self.player.sleeping then 
		if self.options.sleepers then 
			options.boxColor = {options.sleeperColor,1}
			options.box3dColor = {options.sleeperColor,1}
			options.offScreenArrowColor = {options.sleeperColor,1} 
			options.tracerColor = {options.sleeperColor,1}
		else
			enabled = false
		end
	end
...
local EspInterface = {
	...
	teamSettings = {
		enemy = {
			enabled = true,
			sleepers = false,
			sleeperColor = Color3.fromRGB(5, 77, 136),
			box = false,
...
function EspInterface.Load()
	...
	local plrs = EspInterface.getPlayers();
	for model, player in pairs(plrs) do
		createObject(player);
	end
...
function EspInterface.AddPlayer(player)createObject(player) end
function EspInterface.RemovePlayer(player)removeObject(player) end
